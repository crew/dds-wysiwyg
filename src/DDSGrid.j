...
DDSGridTemporaryShowingTime = 1.0;

@implementation DDSGrid : CPObject
{
	CPColor _color;
  float _spacing;
  BOOL _isAlwaysShown;
  ...
  if (_hidingTimer)
{
  ...
  [_hidingTimer setFireDate:[CPDate dateWithTimeIntervalSinceNow:DDSGridTemporaryShowingTime]];
}
  else
{
  // No, so show it the next time -drawRect:inView: is invoked, and then hide it again in one second.
  _hidingTimer = [CPTimer scheduledTimerWithTimeInterval:DDSGridTemporaryShowingTime target:self selector:@selector(stopShowingGridForTimer:) userInfo:nil repeats:NO];
...
- (void)drawRect:(CPRect)rect inView:(CPView)view
{
